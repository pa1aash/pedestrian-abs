...
\begin{enumerate}
    ...
    \item A \textbf{symmetry-breaking control experiment} (C1$+\varepsilon$) that decomposes ORCA's deadlock-resolution effect into approximately 46\% geometric symmetry-breaking (reproducible by passive Gaussian velocity noise) and 54\% velocity-space optimisation (ORCA's coordinated LP-based velocity selection), showing that both mechanisms contribute and neither is sufficient alone.
    \item A \textbf{density-adaptive convex combination} of the three paradigms that mitigates each failure mode, calibrated phenomenologically against 4776 FZJ unidirectional corridor points and evaluated against held-out FZJ bottleneck flow rates, with external validation against JuPedSim.
\end{enumerate}
